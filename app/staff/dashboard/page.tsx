    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 xl:grid-cols-2 gap-6">
            ...
            {todaysAppts.map((a, i) => (
              <div key={a.id} className="flex items-center gap-4 px-5 py-3 hover:bg-slate-50">
                ...
                <div className="flex-1 min-w-0">
                  <p className="font-medium text-sm text-slate-800 truncate">{a.patientName}</p>
                  <p className="text-xs text-slate-400">{a.doctorName}</p>
                </div>
                <div className="text-right">
                  <p className="text-xs font-medium text-slate-700">{formatTime(a.time)}</p>
                  <StatusBadge status={a.status} />
                </div>
              </div>
            ))}
            {todaysAppts.length === 0 && (
              <p className="px-5 py-6 text-sm text-slate-400 text-center">Queue is empty</p>
            )}
          </div>
        </Card>
        ...
        <div className="space-y-3">
          ...
          {[
            { title: 'Register New Patient', desc: 'Add a new patient to the system', href: '/staff/patient-registration', color: 'bg-blue-50 border-blue-100 hover:bg-blue-100' },
            { title: 'Schedule Appointment', desc: 'Book a new appointment slot', href: '/staff/appointments', color: 'bg-emerald-50 border-emerald-100 hover:bg-emerald-100' },
            { title: 'Generate Bill', desc: 'Create an invoice for a patient', href: '/staff/billing', color: 'bg-violet-50 border-violet-100 hover:bg-violet-100' },
          ].map((action) => (
